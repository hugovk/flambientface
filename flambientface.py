...
This program is demonstration for face and object detection using haar-like
features. The program finds faces in a camera image or video stream and
displays a red box around them.
...
def opencv_to_pil(opencv_img):
    return Image.fromstring(
        "RGB", cv.GetSize(opencv_img), opencv_img.tostring())
...
def detect_and_draw(img, cascade):
    ...
    if(cascade):
        ...
        if faces:
            for ((x, y, w, h), n) in faces:
                ...
                # Triangulize!
                t = start_timer()
                pil_image = triangulizor.triangulize(
                    pil_image, options.tile_size)
                stop_timer(t, "triangulize")

                # PIL to OpenCV
                t = start_timer()
                # Depth, channels:
                cropped = cv.CreateImageHeader(
                    pil_image.size, cv.IPL_DEPTH_8U, 3)
                ...
                # cv.Rectangle(
                #    img, (x, y), (x + w, y + h), cv.RGB(255, 0, 0), 3, 8, 0)
    ...
    if options.outfile:
        ...
        outfile = os.path.join(
            TEMP_DIR, TEMP_BASE + str(count).zfill(6) + TEMP_SUFFIX)
...
if __name__ == '__main__':
    ...
    parser = OptionParser(
        usage="usage: %prog [options] [filename|camera_index (hint: try 1)]")
    parser.add_option(
    ...
    parser.add_option(
        "-t", "--tile-size", type=int, default=30,
        help='Tile size (should be divisible by 2. '
        'Use 0 to guess based on image size).')
    parser.add_option(
        "-o", "--outfile", help='Output filename of animated gif')
